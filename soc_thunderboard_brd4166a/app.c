...
#define SERVO_DOOR_UNLOCK 7
#define SERVO_DOOR_LOCK   13

uint8_t pwm_lut[] = {
    SERVO_DOOR_UNLOCK, SERVO_DOOR_LOCK
};
...
void servo_pwm_init(void)
{
  ...
  pwm_config.polarity = PWM_ACTIVE_HIGH;

  sl_pwm_init(&sl_pwm_servo0, &pwm_config);

  sl_pwm_set_duty_cycle(&sl_pwm_servo0, SERVO_DOOR_UNLOCK);

  // Enable PWM output
  sl_pwm_start(&sl_pwm_servo0);
}
...
void sl_button_on_change(const sl_button_t *handle)
{
  ...
  if (sl_button_get_state(handle) == SL_SIMPLE_BUTTON_PRESSED) {
      if (&sl_button_btn0 == handle) {
          sl_app_log("Button 0 pressed, door unlocked\n");
          sl_pwm_set_duty_cycle(&sl_pwm_servo0, SERVO_DOOR_UNLOCK);
      }
      else if(&sl_button_btn1 == handle) {
          sl_app_log("Button 1 pressed, door locked\n");
          sl_pwm_set_duty_cycle(&sl_pwm_servo0, SERVO_DOOR_LOCK);
      }
  }
  else if (sl_button_get_state(handle) == SL_SIMPLE_BUTTON_RELEASED) {
      if (&sl_button_btn0 == handle) {
          sl_app_log("Button 0 released\n");
      }
      else if(&sl_button_btn1 == handle) {
          sl_app_log("Button 1 released\n");
      }
  }
